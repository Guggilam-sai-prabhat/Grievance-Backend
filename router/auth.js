...
router.post("/register",async(req,res)=>{
    ...
    try{
        ...
        if(userExist){
          ...
          if(userRegister){
            const message = {
              from: 'Grievance <postmaster@' + process.env.MAILGUN_DOMAIN + '>',
              to: `${email}`,
      ...
      if (userContact) {
          ...
          mg.messages.create(process.env.MAILGUN_DOMAIN, message)
              ...
              .catch(err => {
                  ...
                  return res.status(500).json({ error: "Failed to send grievance confirmation email" });
              });
      } else {
    ...
    const doc=await User.updateOne({_id},
      ...
      const message = {
        ...
        text: `Hello, ${dept} Department had an update on your grievance number ${gId} and 
               ...
               Find the feedback associated to your grievance: ${feedback}`
      };
      ...
      .catch(err => {
        console.error("Error sending email:", err);
        return res.status(500).json({ error: "Failed to send update notification email" });
      });
